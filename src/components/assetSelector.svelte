<script lang='ts'>
    ...
    function assetSelectFactory(assetdisp:displayAsset){
        const handleAssetSelect =  function(e){
            ...
            selectedAsset = new window.StellarSdk.Asset(assetdisp.code, assetdisp.asset)
...
<div class="flex">
    <div style="flex:1;">
    <Search class="input" style="height:35px;" placeholder="search"  on:focus={()=>{assetSelected = false; DropdownOpen=true;}} on:keyup={handleInputChange} bind:value={currentSearch} on:blur={()=>preventClose?null:DropdownOpen=false}/>
    </div>
    {#if assetSelected}
    <div style="flex:3;">
...
{#if DropdownOpen}
    ...
    {#each assetResults as assetdisp}
        <li>
            <button type="button" class="btn variant-filled w-full" on:mouseenter={()=>preventClose=true} on:mouseleave={()=>preventClose=false} on:click={assetSelectFactory(assetdisp)}>
            <div class='flex-col'>
                <div class='flex'>
